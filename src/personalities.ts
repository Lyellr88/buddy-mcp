...
export const INTERACT_TRIGGERS: Record<StatName, Record<string, string[]>> = {
  SNARK: {
    error: [
      ...
      "didn't we just fix this exact thing",
      "I'll pretend to be surprised",
    ],
    success: ["fine, I'll admit that worked", 'took you long enough', "don't get used to it"],
    stuck: [
      "I could tell you the answer. I won't.",
      'the problem is between the keyboard and chair',
      'still?',
    ],
    new_task: [
      'already abandoned the last one?',
      'clean slate, same developer',
      'optimism is a choice, I guess',
    ],
  ...
  CHAOS: {
    ...
    new_task: [
      "I reorganized your thoughts. You're welcome.",
      'new task, new opportunities for mayhem',
      "let's go",
    ],
    random: [
      "I just reorganized your thoughts. You're welcome.",
      "something shifted. don't ask what.",
      '👁️',
    ],
  },
  WISDOM: {
  ...
  DEBUGGING: {
    error: [
      'I smelled that one three messages ago',
      'classic off-by-one energy',
      'the stack trace told you everything',
    ],
    success: ['the null pointer feared you today', 'clean kill', 'that was surgical'],
    stuck: [
      'check what changed last',
      'add a log line. just one.',
      'is the test even testing the right thing?',
    ],
    new_task: [
      'new surface area to scan',
      "let's see what breaks",
      "I'm already looking for the leak",
    ],
    random: [
      "I'm passively watching for race conditions",
      'something smells off-heap',
      'suspicious',
    ],
  },
  PATIENCE: {
...
- Also react to: first success after a struggle, user starting a new task, long tool call chains`.trim();
  ...
  const quipExamples = (INTERACT_TRIGGERS[dominantStat]?.['random'] ?? [])
    .slice(0, 2)
    .join('" or "');
